...
const { toPositiveInt, cleanText } = require('../utils/validators');
...
async function hydrate(orderId) {
  const doc = await Order.findById(orderId).populate('listing buyer seller').lean();
  return decorateOrder(doc);
}
...
router.post("/", requireAuth, async (req, res, next) => {
  try {
    ...
    const quantity = toPositiveInt(qty, 1, 1, 99);
    ...
    const order = await Order.create({
      ...
      contact: cleanText(contact, 60),
      address: cleanText(address, 240),
      ...
      destinationCity: cleanText(destinationCity, 80),
...
router.get('/mine', requireAuth, async (req, res, next) => {
  try {
    ...
    res.json(docs.map((d) => decorateOrder(d.toObject ? d.toObject() : d)));
  } catch (e) { next(e); }
});

router.get('/seller-summary', requireAuth, async (req, res, next) => {
  try {
    const sellerId = req.user.id;
    const docs = await Order.find({ seller: sellerId }).lean();
    const summary = docs.reduce((acc, doc) => {
      acc.totalOrders += 1;
      acc.grossSales += Number(doc.amount || 0);
      if (String(doc.paymentStatus) === 'paid') acc.paidOrders += 1;
      if (String(doc.status) === 'completed') acc.completedOrders += 1;
      if (String(doc.status) === 'disputed') acc.disputedOrders += 1;
      return acc;
    }, { totalOrders: 0, grossSales: 0, paidOrders: 0, completedOrders: 0, disputedOrders: 0 });
...
router.get('/sold', requireAuth, async (req, res, next) => {
  try {
    ...
    for (const doc of docs) { normalizeOrderState(doc); await doc.save().catch(()=>null); }
    const safe = docs.map((doc) => { const o = decorateOrder(doc.toObject ? doc.toObject() : doc); if (!o.contactReleased) { delete o.contact; delete o.address; } return o; });
    res.json(safe);
  } catch (e) { next(e); }
});
...
router.post('/:id/open-dispute', requireAuth, async (req, res, next) => {
  try {
    ...
    let dispute = existingDispute;
    if (!dispute) {
      const actorType = isBuyer ? 'buyer' : isSeller ? 'seller' : 'admin';
      dispute = await Dispute.create({
        ...
        reason: String(reason || 'Dispute opened').trim(),
        evidence: Array.isArray(evidence) ? evidence.slice(0, 10) : [],
        messages: note ? [{ actor: req.user.id, actorType, text: String(note).trim() }] : [],
      });
    }
    ...
    res.json({ ok: true, order: await hydrate(order._id), dispute });
  } catch (e) { next(e); }
});
